...
import { DiagramSchema, GroupedStock, StockDataResponse, TransformedData, UserInfo } from '../types'
...
export const getStockByIdService = async (
    ...
    try {
        console.log("api service .... ", `${process.env.STOCK_BASE_URL}:${process.env.STOCK_PORT_SERVICE}${process.env.STOCK_PATH}?q=${fieldValue}`)
        const response: AxiosResponse<TransformedData> = await axios.get(`${process.env.STOCK_BASE_URL}:${process.env.STOCK_PORT_SERVICE}${process.env.STOCK_PATH}?q=${fieldValue}`)
...
export const createStockService = async (transformedData: TransformedData, userInfo: object) => {
    try {
        const id = uuidv4()
        ...
        const persistResult = await addFirebaseDocument(diagramData, collectionName)
        if(!persistResult) {
...
export const getStockServiceStats = async (orderByField: string, userInfo: UserInfo) => {
    try {
        const collectionName = process.env.COLLECTION_NAME;
        ...
        if(returnStockStats){
            const stockStats: StockDataResponse[] = returnStockStats.map((doc: DocumentData) => {            
                const stockData = {
                    Name: doc.data.name,
...
const groupStocksBySymbol = (data: StockDataResponse[]): GroupedStock[] => {
    ...
    const sortedMappedStocks = mappedStocks.sort( (a,b) => b.times_requested - a.times_requested)
