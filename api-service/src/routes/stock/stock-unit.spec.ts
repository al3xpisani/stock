...
describe('Stock Unit Tests', () => {
    ...
    it('should return 200 if query parameter is added', async () => {
      (checkValidToken as jest.Mock).mockImplementation((req, res, next) => next());
      const mockData = {
        hasData: true,
        status: {
          name: 'Test Stock',
  ...
  it('should return 400 if query parameter is missing', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ 
      status: "Error when listing Stock TypeError: Cannot read properties of undefined (reading 'q')"
    });
  });
});
